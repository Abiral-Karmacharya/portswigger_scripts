...
class FileUpload: 
    ...
    def login(self):
        ...
        ("Path traversal", self.path_traversal, "path_traversal"),
        ("Blacklist bypass", self.blacklist_bypass, "blacklist_bypass"),
        ("Null byte injection", self.null_byte_injection, "null_byte"),
        ("Magic byte upload", self.magic_byte_file_upload, "magic_byte")
        ]

        for method_name, method, tutorial_name in exploit_methods:
            try:
                if method():
                    burpsuite = False
                    print(f"Success with {method_name}")
                    if tutorial_name != "simple_file_upload":
                        burpsuite=True
                    self.solve_tutorial(exploit_condition=True, type_of_exploit=tutorial_name, burpsuite=burpsuite)
                    print("Bye bye ˙◠˙")
                    return True
            except Exception as e:
